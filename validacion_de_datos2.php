<?php
	...
	if(!mysqli_connect_error()) {
		// Consulta a la db
		$consulta  = "SELECT * FROM usuario WHERE nombreUsuario = '$usuarioEnviado' and password = '$passwordEnviado'";
		$respuesta = mysqli_query($conexion->conectado,$consulta);	// Respues de la db con la conexion y la consulta

		$matriz = array();					// Array de objetos donde guardo los datos de los usuario
		
		while ($obj = mysqli_fetch_object($respuesta)) {
			$matriz[] = array("id" => $obj -> id_usuario, "usuario" => utf8_encode($obj -> nombreUsuario));
		}
